...
| **C** | 已有网格 → 后处理 | 任何来源的粗糙网格拖进来只跑修复/减面/UV/烘焙/导出。零 API 成本，是低门槛获客入口，也是让美术信任校验器的方式 |
...
┌───────▼──────────────────────────────────────┐
│ meshy │ tripo │ rodin │ mock │ local Trellis   │
└──────────────────────────────────────────────┘
```
...
**BYOK**：生成功能需要自备 Provider API Key，在设置页填写（OS 级加密存储，永不上传）。不填 Key 时**工作流 C 完整可用**，生成入口会被禁用并给出引导。
